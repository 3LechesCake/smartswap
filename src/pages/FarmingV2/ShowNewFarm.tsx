...
const ShowNewFarm = ({content, wallet, URLReferrerAddress, LoadingState, section, showYieldfarm, contractID}: {
    ...
    const RGPSpecialPoolV1Approval = async () => {
        if (account) {
            try {
                ...
                if (confirmations >= 3) {
                    setApproveValueForRGP(true);
                    GFarmingSuccessTransaction("special pool", "approval",  "TRES","v1")
                    dispatch(
                        setOpenModal({
                            trxState: TrxState.TransactionSuccessful,
                            message: `Successful TRES Approval`,
                        })
                    );
                }
                getAllowances();
                ...
                console.error(error);
                GFarmingFailedTransaction("special pool", "approval", error.message, "TRES","v1")
                dispatch(
                    setOpenModal({
    ...
    const RGPSpecialPoolV2Approval = async () => {
        if (account) {
            try {
                ...
                if (confirmations >= 3) {
                    setApproveValueForRGP(true);
                    GFarmingSuccessTransaction("special pool", "approval", "TRES","v2")
                    dispatch(
                        setOpenModal({
                            trxState: TrxState.TransactionSuccessful,
                            message: `Successful TRES Approval`,
                ...
                console.error(error);
                GFarmingFailedTransaction("special pool", "approval", error.message, "TRES","v2")
                dispatch(
                    setOpenModal({
                        message: `Transaction failed`,
                        trxState: TrxState.TransactionFailed,
    ...
    async function confirmUnstakeDeposit(val: string) {
        try {
            ...
            if (account) {
                if (val === "TRES" && Number(content.id) === 1) {
                    await RGPUnstake(val);
                } else if (val === "TRES" && Number(content.id) === 13) {
    ...
    const harvestTokens = async (id: string | number) => {
        if (account) {
            try {
                ...
                dispatch(
                    setOpenModal({
                        message: `Harvesting TRES ${content.RGPEarned} Tokens`,
                        trxState: TrxState.WaitingForConfirmation,
                    })
                );
                if (id === 0) {
                    ...
                    if (confirmations >= 1 && status) {
                        GFarmingSuccessTransaction("special pool", "harvest", "TRES","v1")
                        dispatch(
                            setOpenModal({
                                trxState: TrxState.TransactionSuccessful,
                    ...
                    if (confirmations >= 1 && status) {
                        GFarmingSuccessTransaction("special pool", "harvest", "TRES","v2")
                        dispatch(
                            setOpenModal({
                                trxState: TrxState.TransactionSuccessful,
                                message: `Successfully Harvested ${convertFromWei(
                    ...
                    if (confirmations >= 1 && status) {
                        GFarmingSuccessTransaction("farming", "harvest", "TRES","v2")
                        dispatch(
                            setOpenModal({
                                trxState: TrxState.TransactionSuccessful,
                                message: `Successfully Harvested ${convertFromWei(
                ...
                GFarmingFailedTransaction( "special pool","harvest", error.message, "TRES","v2")
                dispatch(
                    setOpenModal({
                        trxState: TrxState.TransactionFailed,
                ...
                if (parseFloat(content.tokenStaked[1]) == 0) {
                    if (parseFloat(RGPBalance) < parseFloat(farmingFee)) {
                        ...
                        dispatch(
                            setOpenModal({
                                trxState: TrxState.TransactionFailed,
                                message: `Insufficient TRES, you need at least ${farmingFee} TRES to enter this pool`,
                            })
                        );
                    } else {
                        const lpTokens = await MasterChefNEWLPContract(
                    ...
                    setDeposited(true);
                    setReload(true);
                    setContentId(content.deposit === "TRES" ? undefined : content.id);
                    //  callRefreshFarm(confirmations, status);
                }
            } catch (error: any) {
    ...
    const RGPuseStake = async () => {
        if (account) {
            try {
                ...
                const { confirmations, status } = await fetchTransactionData(data);
                GFarmingSuccessTransaction("special pool", "staking", "TRES","v1")
                dispatch(
                    setOpenModal({
                        trxState: TrxState.TransactionSuccessful,
                        message: `Successfully staked ${depositTokenValue} TRES `,
                    })
                );
                // callRefreshFarm(confirmations, status);
            } catch (error:any) {
                console.log(error);
                GFarmingFailedTransaction("special pool", "stake", error.message, "TRES","v1")
                dispatch(
                    setOpenModal({
                        message: `Transaction failed`,
                        trxState: TrxState.TransactionFailed,
    ...
    const RGPuseStakeV2 = async () => {
        if (account) {
            try {
                ...
                GFarmingSuccessTransaction("special pool", "stake", "TRES","v2")
                dispatch(
                    setOpenModal({
                        trxState: TrxState.TransactionSuccessful,
                        message: `Successfully staked ${depositTokenValue}TRES `,
                ...
                console.log(error);
                GFarmingFailedTransaction("special pool", "stake", error.message, "TRES","v2")
                dispatch(
                    setOpenModal({
                        message: `Transaction failed`,
                        trxState: TrxState.TransactionFailed,
    ...
    const RGPUnstake = async () => {
        if (account) {
            try {
                ...
                const { confirmations, status } = await fetchTransactionData(data);
                GFarmingSuccessTransaction("special pool", "unstake", "TRES","v1")
                dispatch(
                    setOpenModal({
                        trxState: TrxState.TransactionSuccessful,
                        message: `Successfully unstaked ${unstakeToken} TRES `,
                    })
                );
                // dispatch the getTokenStaked action from here when data changes
                //  callRefreshFarm(confirmations, status);
                ...
                console.log(e);
                GFarmingFailedTransaction("special pool", "unstake", e.message, "TRES","v1")
                dispatch(
                    setOpenModal({
                        trxState: TrxState.TransactionFailed,
                        message: `Transaction was not successful`,
    ...
    const RGPUnstakeV2 = async (val:string) => {
        if (account) {
            try {
                ...
                const { confirmations, status } = await fetchTransactionData(data);
                GFarmingSuccessTransaction("special pool", "unstake", "TRES","v2")
                dispatch(
                    setOpenModal({
                        trxState: TrxState.TransactionSuccessful,
                        message: `Successfully unstaked ${unstakeToken} TRES `,
                ...
                GFarmingFailedTransaction("special pool", "unstake", error.message, "TRES","v2")
                dispatch(
                    setOpenModal({
                        trxState: TrxState.TransactionFailed,
                        message: `Transaction was not successful`,
    ...
    const RGPApproval = async () => {
        if (account) {
            try {
                ...
                if (confirmations >= 3) {
                    setApproveValueForRGP(true);
                    GFarmingSuccessTransaction("farming", "approval", "TRES","v2")
                    dispatch(
                        setOpenModal({
                            trxState: TrxState.TransactionSuccessful,
                            message: `Successful TRES Approval`,
                ...
                console.error(error);
                GFarmingFailedTransaction("farming", "approval", error.message ,"TRES","v2")
                dispatch(
                    setOpenModal({
                        message: `Transaction failed`,
                        trxState: TrxState.TransactionFailed,
    ...
    const bgColor2 = useColorModeValue("#ce0014", "#4CAFFF");
                                ...
                                <Box>
                                    ...
                                    <Flex marginLeft={{ base: "20px", md: "none", lg: "none" }}>
                                        <Button
                                            ...
                                            h='40px'
                                            borderRadius='6px'
                                            bg={mode === DARK_THEME ? "#4CAFFF" : "#ce0014"}
                                ...
                                {isMobileDevice ? (
                                    <Box width='60%' margin='0 auto'>
                                        ...
                                        <Button
                                            ...
                                            onClick={() => {
                                                harvestTokens(
                                                    content.deposit === "TRES" ? content.pId : content.id
                                                );
                                            }}
                                            className={"harvest"}
                                        >
                        ...
                        >
                            ...
                            >
                                ...
                                <ModalBody py={2}>
                                    ...
                                    <InputGroup size='md'>
                                        <Input
                                            placeholder='Enter TRES amount to stake'
                                            opacity='0.5'
                                            h='50px'
                                            borderRadius='6px'
                                            name='availableToken'
                                    ...
                                    <Text color={modalTextColor2} fontSize='14px' mb={5} mt={3}>
                                        TRES Available: {content.availableToken} {content.deposit}
                                    </Text>
