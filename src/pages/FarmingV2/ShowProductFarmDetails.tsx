...
const ShowProductFarmDetails = ({
  ...
  useEffect(() => {
    ...
    const checkForApproval = async () => {
      ...
      // if (content.deposit === "TRES" && content.feature === "AT") {
  ...
  const FarmProductApproval = async () => {
    if (account) {
      try {
        ...
        if (confirmations >= 3) {
          setApproveValueForRGP(true);
          GFarmingSuccessTransaction("product farm | auto period", "approval", "TRES","v1")
          dispatch(
            setOpenModal({
              trxState: TrxState.TransactionSuccessful,
        ...
        console.error(error);
        GFarmingFailedTransaction("product farm | auto period", "approval", error.message, "TRES","v1")
        dispatch(
          setOpenModal({
  ...
  const checkUser = async (val :string) => {
    if (content.deposit === "TRES" && content.feature) {
    ...
    if (account) {
      try {
       ...
       GFarmingSpecialPoolReferral(referrerAddress===ZERO_ADDRESS ? false:true)
        ...
        GFarmingSuccessTransaction("Product farm | auto period", "stake", "TRES","v1")
        refreshSpecialData()
        dispatch(
          setOpenModal({
            trxState: TrxState.TransactionSuccessful,
        ...
        console.log(error);
        GFarmingFailedTransaction("Product farm | auto period", "stake", error.message, "TRES","v1")
        dispatch(
          setOpenModal({
            message: `Transaction failed`,
            trxState: TrxState.TransactionFailed,
