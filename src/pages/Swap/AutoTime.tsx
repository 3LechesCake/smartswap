 ...
 useEffect(()=>{
  ...
  useEffect(async () => {
    const checkBalance = async ()=>{
     if(currencies[Field.INPUT]?.symbol==="TRES"){
  ...
  const checkForApproval = async () => {
    ...
    if (parseFloat(RGPBalance) >= parseFloat(fee)) {
      ...
      setApprovalForFee("TRES")
    }
    if((parseFloat(tokenBalance) >= (parseFloat(formattedAmounts[Field.INPUT]) * frequency)+parseFloat(fee)) && currencies[Field.INPUT]?.wrapped.symbol === "TRES"){
      setHasBeenApproved(true)
      // setApprovalForFee("")
      setApprovalForToken("")
    }
    ...
    if (parseFloat(RGPBalance) < parseFloat(fee) || (parseFloat(tokenBalance) < parseFloat(formattedAmounts[Field.INPUT]) * frequency && (!currencies[Field.INPUT]?.isNative && currencies[Field.INPUT]?.wrapped?.symbol === "TRES" ))) {
      setHasBeenApproved(false)
      setApprovalForFee("TRES")
      setApprovalForToken("TRES")
    }
    if (parseFloat(tokenBalance) < parseFloat(formattedAmounts[Field.INPUT]) * frequency && (!currencies[Field.INPUT]?.isNative &&currencies[Field.INPUT]?.wrapped.symbol !== "TRES")) {
      setHasBeenApproved(false)
  ...
  const signTransaction = async () => {
    if (account !== undefined) {
      // try {
        ...
        const permitHash = "0xad995048297da359fa21ab96c71c12abe86713fd2567e63d116a3414a7846800";
  ...
  const approveOneOrTwoTokens = async (tokenApprovingFor:string) => {
    ...
    GButtonClick("auto_period",`Approve ${tokenApprovingFor} ${tokenApprovingFor ==="TRES" ? "for fee" : ""}`,currencies[Field.INPUT]?.symbol)

        dispatch(
          setOpenModal({
            message: `Approve ${tokenApprovingFor} ${tokenApprovingFor ==="TRES" ? "for fee" : ""}`,
            trxState: TrxState.WaitingForConfirmation,
          })
        );
        try{

        if (tokenApprovingFor === "TRES") {
        ...
        const frequency = parseInt(totalNumberOfTransaction) > 1 ? parseInt(totalNumberOfTransaction) : 1
         ...
         let walletBalString = parseFloat(ethers.utils.formatUnits(walletBal.toString(), currencies[Field.INPUT]?.decimals)) 
          ...
          // setArr =setArr.filter(item=>item!=="TRES")
          ...
          GSuccessfullyTransaction("auto_period",`Approval ${tokenApprovingFor} ${tokenApprovingFor ==="TRES" ? "for fee" : ""}`,currencies[Field.INPUT]?.symbol)
            setApprovalForFee("")
    ...
    <Box fontSize="xl">
      ...
      <MenuList>
        {["5","30","60","daily","weekly","monthly"].map((item:string,index)=>(
          <MenuItem key={index} _focus={{ color: "#ce0014" }} onClick={(e) => setSelectedFrequency(item)} fontSize="13px">
          ...
          >
            <TabList>
              <Tab
                // padding="8px 34px"
                padding="7px"
                background={situation==="above" ? "#ce0014" : ""}
                color={situation!=="above" ? "#ce0014" : ""}
                borderRadius="4px"
                border="none"
              ...
              <Tab
                padding="7px"
                background={situation==="below" ? "#ce0014" : ""}
                color={situation!=="below" ? "#ce0014" : ""}
                border="none"
                borderRadius="4px"
              ...
              >
                ...
                <Flex mt={10} justifyContent="space-between">
                  <Text fontSize="16px">Fee:</Text> <Text fontSize="16px" opacity="0.7" ml={1}>{fee} TRES</Text>
                </Flex>
                <VStack>
                  <Flex>
                    <Text fontSize="14px" mr={2}>
            ...
            >
              {inputError
                ? inputError
                : `Insufficient ${currencies[Field.INPUT]?.symbol} Balance ${currencies[Field.INPUT]?.symbol==="TRES" ? "for fee":""}`}
