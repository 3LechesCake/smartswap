 ...
 if(parseFloat(initialToPrice) >0){
  ...
  const approveOneOrTwoTokens = async (tokenApprovingFor:string) => {
    ...
    const checkForApproval = async () => {
      ...
      if (parseFloat(RGPBalance) < parseFloat(fee) || (parseFloat(tokenBalance) < Number(formattedAmounts[Field.INPUT]) && (!currencies[Field.INPUT]?.isNative && currencies[Field.INPUT]?.wrapped?.symbol === "TRES" ))) {
        setHasBeenApproved(false)
        approvalArray.push("TRES")
        setApprovalForFee("TRES")
        setApprovalForToken("TRES")
      }
      if (parseFloat(tokenBalance) < Number(formattedAmounts[Field.INPUT]) && (!currencies[Field.INPUT]?.isNative &&currencies[Field.INPUT]?.wrapped.symbol !== "TRES")) {
        setHasBeenApproved(false)
