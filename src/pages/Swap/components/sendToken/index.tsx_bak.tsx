...
const SendToken = () => {
  ...
  const swapTokens = async () => {
    if (chainId === SupportedChainId.POLYGONTEST || chainId === SupportedChainId.POLYGON) {
      ...
      chainId === SupportedChainId.PHOENIX ||
      chainId === SupportedChainId.PHOENIX
    ) {
      if (
        ...
        await withdraw();
      } else if (currencies[Field.INPUT]?.symbol === "PHX") {
        await swapDefaultForOtherTokens();
      } else if (currencies[Field.OUTPUT]?.symbol === "PHX") {
        await swapOtherTokensForDefault();
      } else {
        await swapDifferentTokens();
      }
      ...
      chainId === SupportedChainId.DOGEMAINNET ||
      chainId === SupportedChainId.DOGEMAINNET
    ) {
      if (
        currencies[Field.INPUT]?.symbol === "DOGE" &&
        currencies[Field.OUTPUT]?.symbol === "WDOGE"
      ) {
        await deposit();
      } else if (
        currencies[Field.INPUT]?.symbol === "WDOGE" &&
        currencies[Field.OUTPUT]?.symbol === "DOGE"
      ) {
        await withdraw();
      } else if (currencies[Field.INPUT]?.symbol === "DOGE") {
        await swapDefaultForOtherTokens();
      } else if (currencies[Field.OUTPUT]?.symbol === "DOGE") {
        await swapOtherTokensForDefault();
      } else {
        await swapDifferentTokens();
      }
    } else if (
      chainId === SupportedChainId.TRESTESTNET ||
      chainId === SupportedChainId.TRESMAINNET
    ) {
      if (
        currencies[Field.INPUT]?.symbol === "TRES" &&
        currencies[Field.OUTPUT]?.symbol === "WTRES"
      ) {
        await deposit();
      } else if (
        currencies[Field.INPUT]?.symbol === "WTRES" &&
        currencies[Field.OUTPUT]?.symbol === "TRES"
      ) {
        await withdraw();
      } else if (currencies[Field.INPUT]?.symbol === "TRES") {
        await swapDefaultForOtherTokens();
      } else if (currencies[Field.OUTPUT]?.symbol === "TRES") {
        await swapOtherTokensForDefault();
      } else {
        await swapDifferentTokens();
      }
