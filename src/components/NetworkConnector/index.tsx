...
import PHXLOGO from "../../assets/phx.png";
...
function NetworkIndicator() {
  ...
  useEffect(() => {
    ...
    if (!connect && param) {
      ...
      } else if (param === "dogechain") {
        handleUpdateChainId(2000);
      } else if (param === "phoenix") {
        handleUpdateChainId(13381);
      } else if (param === "tres") {
        handleUpdateChainId(6066);
      } else if (param === "tres_test") {
        handleUpdateChainId(6065);
      } else {
        history.push(`${location.pathname}?chain=bsc`);
      }
    }
      ...
      >
        <Flex alignItems="center">
          <Box mr={2}>
            ...
            ) : info.label == "Dogechain" ? (
              <Img w="30px" src={DOGELOGO} />
              ) : info.label == "Tres Leches Testnet" ? (
                <Img w="30px" src={TRESLOGO} />
              ) : info.label == "Tres Leches Mainnet" ? (
                <Img w="30px" src={TRESLOGO} />
            ) : (
              <EthereumIcon />
            )}
        ...
        <ModalContent>
          <Flex flexDirection="column" mx={5}>
            ...
            <Flex
              ...
              mb={4}
              cursor="pointer"
              onClick={() => changeNetwork("0X17B2", 6065)}
            >
              <Flex>
                <Box px={2}>
                  <Img w="30px" src={TRESLOGO} />
                </Box>
                <Box>{CHAIN_INFO[6065].label}</Box>
              </Flex>
            </Flex>
            <Flex
              backgroundColor={mode === "dark" ? "#15202B" : "#FFFFFF"}
              ...
              mb={4}
              cursor="pointer"
              onClick={() => changeNetwork("0X17B3", 6066)}
            >
              <Flex>
                <Box px={2}>
                  <Img w="30px" src={TRESLOGO} />
                </Box>
                <Box>{CHAIN_INFO[6066].label}</Box>
              </Flex>
            </Flex>
          </Flex>
        </ModalContent>
